...
(function($)
{
	...
	//  This prevents the widgets from being too tall and wide.
  	...
  	//  indicate whether it's value is current (used by AnyTime.widget).

  	jQuery.prototype.AtwCurrent = function(isCurrent)
	{
	    if ( isCurrent )
	    {
		  this.removeClass('ui-state-default ui-state-highlight');
	      this.addClass('AtwCurrentBtn ui-state-default ui-state-highlight');
	    }
	    else
	      this.removeClass('AtwCurrentBtn ui-state-highlight');
	};
...
AnyTime.widget = function( id, options )
{
	...
	__widgets[id] =
	{
		    ...
		    else if ( ( ( key == 86 ) || ( key == 118 ) ) && event.ctrlKey )
		    {
		    	...
		    	setTimeout(
		    		function()
		    		{
		    			_this.showCal(null);
		    			/*
		    			try { _this.conv.parse(_this.inp.val()); }
		    			catch(e) { _this.inp.val(""); }
		    			*/
		    		}, 100 );	
		    	return;
		    }
		...
		upd: function(fBtn)
		{
		          ...
		          function()
		          {
		        	  if ( dow - _this.fDOW < 7 )
		        	  {
				          ...
				          else
				          {
				            ...
				            td.AtwCurrent( dom.getDate() == current );
				            dom.setDate(dom.getDate()+1);
				          }
		        	  }
		              dow++;
		    ...
		    $('#'+this.id+' .AtwMinTenBtn').each(
		      function()
		      {
		        $(this).AtwCurrent( this.innerHTML == String(tens) );
		      } );
		    $('#'+this.id+' .AtwMinOneBtn').each(
		      function()
		      {
		        $(this).AtwCurrent( this.innerHTML == String(ones) );
		      } );
		...
		updYDiv: function(fBtn)
		{
		    ...
		    this.yDiv.find('.AtwMilBtn').each(
		      function()
		      {
		        $(this).AtwCurrent( this.innerHTML == yearValue.substring(0,1) );
		      } );
		    this.yDiv.find('.AtwCentBtn').each(
		      function()
		      {
		        $(this).AtwCurrent( this.innerHTML == yearValue.substring(1,2) );
		      } );
		    this.yDiv.find('.AtwDecBtn').each(
		      function()
		      {
		        $(this).AtwCurrent( this.innerHTML == yearValue.substring(2,3) );
		      } );
		    this.yDiv.find('.AtwYrBtn').each(
		      function()
		      {
		        $(this).AtwCurrent( this.innerHTML == yearValue.substring(3) );
		      } );
		    this.yDiv.find('.AtwBCEBtn').each(
		      function()
		      {
		        $(this).AtwCurrent( era < 0 );
		      } );
