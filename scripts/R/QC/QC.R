...
result <- foreach(i=which(ttr$keep == 1), .combine = 'comb', .multicombine = T, .packages = c('gmt','raster','rgeos','sp','rgdal','gdistance')) %dopar% {
	...
	##### Read raw data file
	s <- which(ttr$tag_id == ttr$tag_id[i] & ttr$release_id == ttr$release_id[i]); s_m <- which(tag_metadata$release_id == ttr$release_id[i])
	...
	outcome[i,11] <- ifelse(1.2 * unique(tag_metadata$tag_expected_life_time_days[s_m]) - as.numeric(difftime(temporal_outcome$detection_timestamp[nrow(temporal_outcome)], temporal_outcome$detection_timestamp[1], units = 'days')) < 0, TRUE, FALSE) ## Tags detected longer than expected life time?
