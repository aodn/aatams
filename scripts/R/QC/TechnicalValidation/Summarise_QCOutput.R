...
##### Merge summary statistics and tag metadata
	...
	for (i in 1:nrow(dat)){
		...
		if (i == 1) {dat_all <- cbind(dat[i,], metadata[s,c(4,6:22)])} else {dat_all <- rbind(dat_all, dat_all <- cbind(dat[i,], metadata[s,c(4,6:22)]))}
	}
	dat <- dat_all; rm(dat_all);
	write.table(dat, paste(wd,'/Outcomes/QC_OutputSummary.csv', sep = ''), row.names=F, col.names=T, sep = ';');

##### Re-assign spangled emperor to Spangled Emperor
	...
	dat$scientific_name[which(dat$scientific_name == 'Lethrinus nebulosus & Lethrinus sp.')] <- 'Lethrinus nebulosus'; dat$common_name[which(dat$common_name == 'spangled emperor')] <- 'Spangled Emperor';
	metadata$scientific_name[which(metadata$scientific_name == 'Lethrinus nebulosus & Lethrinus sp.')] <- 'Lethrinus nebulosus'; metadata$common_name[which(metadata$common_name == 'spangled emperor')] <- 'Spangled Emperor';
...
##### Exclude data for tags that are currently protected or embargoed
	...
	print(paste('Number of registered tags = ', nrow(dat), '; Number of valid detections for registered tags = ', round(sum(dat$nb_detections[which(dat$nb_valid != 0)])/10^6,1), ' million detections', sep = ''));
