...
<xsl:stylesheet version="2.0" xmlns="http://www.w3.org/1999/xhtml"
	...
	<xsl:template match="/">
		<xsl:for-each select="//table">
			...
			<xsl:result-document
				href="{concat('file:///C:/eclipse_workspace/aatams/forms3/create_',lower-case(@name),'.xml')}"
				format="xml">
	...
	<xsl:template name="selected-subfeatures">
		<xf:instance id="subf">
			<data xmlns="">
				<xsl:apply-templates select="foreign-key" mode="selected-subfeature-ids"/>
	...
	<xsl:template match="foreign-key[@foreignTable = 'PROJECT_PERSON']" mode="selected-subfeature-ids">
		<project_id xmlns=""/>
		<project_person_id xmlns=""/>
	</xsl:template>
	
	<xsl:template match="foreign-key[@foreignTable = 'INSTALLATION_DEPLOYMENT']" mode="selected-subfeature-ids">
