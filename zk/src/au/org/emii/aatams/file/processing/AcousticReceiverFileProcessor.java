...
public final class AcousticReceiverFileProcessor {
	...
	public void doProcessing(Connection connection, String downloadFid, String filePath, 
		...
		try {
			...
			PreparedStatement pst = conn.prepareStatement("SELECT DEVICE_ID FROM AATAMS.DEVICE WHERE CODE_NAME = ? AND DEVICE_TYPE_ID = 2");
			ResultSet rset;
			boolean first = true;
	...
	private class DetectionsInserterAndReportGenerator extends Thread {
		...
		public void run() {
			...
			try {
				...
				smt = conn.createStatement();
				if(installationId > 0){
					rset = smt.executeQuery("SELECT NAME FROM AATAMS.INSTALLATION WHERE INSTALLATION_ID = " + installationId);
				    if(rset.next()){
				...
				if(installationStationId > 0){
					rset = smt.executeQuery("SELECT NAME, CURTAIN_POSITION FROM AATAMS.INSTALLATION_STATION WHERE STATION_ID = " + installationStationId);
				    if(rset.next()){
				...
				if(receiverDeploymentId > 0){
					rset = smt.executeQuery("SELECT NAME FROM AATAMS.INSTALLATION_DEPLOYMENT WHERE DEPLOYMENT_ID = " + receiverDeploymentId);
				    if(rset.next()){
				...
				if(projectRolePersonId > 0){
					rset = smt.executeQuery("SELECT PERSON_ROLE FROM AATAMS.PROJECT_PERSON WHERE PROJECT_ROLE_PERSON_ID = " + projectRolePersonId);
				    if(rset.next()){
				...
				try {
					...
					if (detections.size() > 0) {
						...
						// already exist.
						smt.execute("UPDATE AATAMS.DEPLOYMENT_DOWNLOAD SET\n" +
						...
						rset = smt.executeQuery("SELECT COUNT(*) FROM AATAMS.TAG_RELEASE WHERE DEVICE_ID IN (" + list + ")");
						if (rset.next() && rset.getInt(1) > 0) {
							...
							String qry = "SELECT TAG_RELEASE.RELEASE_ID, DEVICE.CODE_NAME, CLASSIFICATION.NAME,\n"
									...
									+ "FROM AATAMS.TAG_RELEASE INNER JOIN AATAMS.DEVICE ON TAG_RELEASE.DEVICE_ID = DEVICE.DEVICE_ID\n"
									+ "INNER JOIN AATAMS.PROJECT_PERSON ON TAG_RELEASE.PROJECT_ROLE_PERSON_ID = PROJECT_PERSON.PROJECT_ROLE_PERSON_ID\n"
									+ "LEFT OUTER JOIN AATAMS.CLASSIFICATION ON TAG_RELEASE.CLASSIFICATION_ID = CLASSIFICATION.CLASSIFICATION_ID\n"
			...
			try {
				smt = conn.createStatement();
				rset = smt.executeQuery("SELECT PERSON.NAME, PERSON.EMAIL FROM AATAMS.PERSON, AATAMS.PROJECT_ROLE_PERSON\n"
			...
			for (Iterator<Entry<Integer, StringBuffer>> i = projectTags.entrySet().iterator(); i.hasNext();) {
				...
				try {
					smt = conn.createStatement();
					rset = smt
							.executeQuery("SELECT PERSON.NAME, PERSON.EMAIL FROM AATAMS.PERSON, AATAMS.PROJECT_ROLE_PERSON\n"
									+ "WHERE PERSON.PERSON_ID = PROJECT_ROLE_PERSON.PERSON_ID AND\n" + "PROJECT_ROLE_PERSON.PROJECT_ROLE_PERSON_ID = "
									+ entry.getKey());
					if (rset.next()) {
